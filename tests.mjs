...
for(const n of legacy.notes){if(n.rest)continue;for(let t=n.startBeat*spb;t<(n.startBeat+n.duration)*spb;t+=.05)samples.push({t:t+.02,hz:midiToFreq(n.midi),clarity:.95,rms:.1});}
const legacyScore=scoreAttempt(legacy,samples,0);assert(legacyScore.pitch>95&&legacyScore.time>90&&legacyScore.flow>90,'legacy scoring regression');

// AudioContext clock is the musical clock.
const fakeCtx={currentTime:10},transport=createTransport({audioContext:fakeCtx,bpm:60,beatsPerBar:4});transport.startAt(14);
...
for(const stage of STAGES.map(s=>s.stage)){
  ...
  assert(plan.events[0].presentationMode==='TEACHER_CALL',`stage ${stage} teacher call`);assert(plan.events[0].modelStartBeat===plan.events[0].startBeat,`stage ${stage} model start`);assert(plan.events.some(x=>x.presentationMode==='BUILD'&&x.morph?.active),`stage ${stage} build morph`);
  for(const x of plan.events){
    assert(x.startBeat%16===0,`${x.eventId} slot`);assert(x.singStartBeat===x.startBeat+8,`${x.eventId} sing start`);assert(x.singEndBeat-x.singStartBeat===x.scoreModel.totalBeats,`${x.eventId} singing window matches score`);assert([4,8].includes(x.scoreModel.totalBeats),`${x.eventId} score size`);assert(x.singEndBeat<=x.endBeat,`${x.eventId} fits learning event`);assert(Array.isArray(x.harmonyTimeline)&&x.harmonyTimeline.length>=1,`${x.eventId} event owns harmony timeline`);assert(x.harmonyFieldId,'Learning Event owns harmony field selection');
  }
}

// Teaching/recovery stays continuous.
const curriculumPlan=buildDailySessionPlan({currentStage:3,eventCount:20}),teacher=curriculumPlan.events.find(x=>x.presentationMode==='TEACHER_CALL');
assert(modelSchedule({scoreModel:teacher.scoreModel,startBeat:teacher.modelStartBeat}).every(x=>x.beat>=teacher.startBeat&&x.beat<teacher.prepareBeat),'four-beat teacher model stays in bar 1');
