...
import {applyKeyTransferResult,deriveKeyProgress,defaultKeyProgress,normalizeKeyProgress} from './curriculum/keyMastery.js';
import {STAGES} from './curriculum/stages.js';
const KEY='bebop-reader-state-v3',LEGACY_KEY='bebop-reader-state-v2';
const defaults={version:3,familyMastery:{},variantHistory:{},stageProgress:{currentStage:0,unlockedStages:[0],advanced:false},keyProgress:defaultKeyProgress(),reviewQueue:[],streak:0,lastPracticeDate:null,totalSessions:0,settings:{solfege:false,preferredBpm:72,latencyMs:0},currentSession:null,lastSessionResult:null,migratedFromV2:false};
const clone=x=>JSON.parse(JSON.stringify(x));
function hydratedState(raw={}){
 const state={...clone(defaults),...raw,settings:{...defaults.settings,...(raw.settings||{})},stageProgress:{...defaults.stageProgress,...(raw.stageProgress||{})},familyMastery:{...(raw.familyMastery||{})}};
 state.keyProgress=deriveKeyProgress(normalizeKeyProgress(raw.keyProgress),{stageProgress:state.stageProgress,familyMastery:state.familyMastery});
 return state;
}
export function migrateV2State(v2={}){return hydratedState({...clone(defaults),streak:Number(v2.streak)||0,lastPracticeDate:v2.lastPracticeDate||null,settings:{...defaults.settings,...(v2.settings||{})},migratedFromV2:true});}
export function loadStateV3(){
 try{
  const saved=localStorage.getItem(KEY);if(saved)return hydratedState(JSON.parse(saved));
 ...
 }catch{return hydratedState(defaults);}
}
...
export function recordSessionEventV3(event,result){return mutateV3(s=>{
 const now=Date.now(),id=event.familyId;
 if(event?.keyTransfer&&event?.key&&event.key!=='C'){
  s.keyProgress=applyKeyTransferResult(s.keyProgress,event,result,s.currentSession?.sessionId,now);
 }else{
  if(id)s.familyMastery[id]=applyEventResult(s.familyMastery[id],event,result,now);
