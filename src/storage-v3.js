...
export const defaultKeyProgress=()=>({C:{unlocked:true},F:{unlocked:false},Bb:{unlocked:false}});
export function normalizeKeyProgress(raw={}){
 const out=defaultKeyProgress();
 for(const key of ['C','F','Bb']){
  const value=raw?.[key];
  if(value&&typeof value==='object')out[key]={...out[key],...value,unlocked:Boolean(value.unlocked)};
  else if(typeof value==='boolean')out[key]={...out[key],unlocked:value};
 }
 out.C.unlocked=true;
 return out;
}
...
export function loadStateV3(){
 try{
  const saved=localStorage.getItem(KEY);if(saved){const raw=JSON.parse(saved);return{...clone(defaults),...raw,settings:{...defaults.settings,...(raw.settings||{})},stageProgress:{...defaults.stageProgress,...(raw.stageProgress||{})},keyProgress:normalizeKeyProgress(raw.keyProgress)};}
