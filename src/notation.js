...
function percentAtBeat(timeline,ex,beat){
  const points=timeline?.points||[];if(!points.length)return 14+beat/ex.totalBeats*78;
  ...
  const last=points[points.length-1],t=clamp((beat-last.beat)/Math.max(.001,ex.totalBeats-last.beat));
...
function drawMeasureLines(svg,timeline,ex){
  ...
  for(let beat=4;beat<ex.totalBeats;beat+=4){
    ...
    line.setAttribute('x1',x);line.setAttribute('x2',x);line.setAttribute('y1','48');line.setAttribute('y2','88');
    line.setAttribute('stroke','#111');line.setAttribute('stroke-width','1.5');line.setAttribute('data-barline','true');
    svg.appendChild(line);
  }
}
function makeBeams(VF,vnotes,notes,ctx){
  const groups=[];let group=[],beatBucket=null;
  const flush=()=>{if(group.length>=2)groups.push(group);group=[];beatBucket=null;};
  notes.forEach((n,i)=>{
    const bucket=Math.floor(n.startBeat+1e-6);
    if(!n.rest&&n.duration===.5){
      if(beatBucket!==null&&bucket!==beatBucket)flush();
      beatBucket=bucket;group.push(vnotes[i]);
    }else flush();
  });flush();
  groups.forEach(g=>{try{new VF.Beam(g).setContext(ctx).draw();}catch{}});
}
export function renderNotation(container,ex,{resultNotes=null,showTime=true}={}){
  ...
  new VF.Formatter().joinVoices([voice]).format([voice],W-120);voice.draw(ctx,stave);makeBeams(VF,vnotes,ex.notes,ctx);
  ...
  const lastPoint=points[points.length-1];const endX=clamp((lastPoint?.x||90)+Math.max(2,Math.min(7,(lastPoint?.duration||1)*2.5)),4,97);const timeline={points,endX};container.dataset.timeline=JSON.stringify(timeline);
  const svg=container.querySelector('svg');if(svg){svg.setAttribute('viewBox',`0 0 ${W} ${H}`);svg.setAttribute('width','100%');svg.removeAttribute('height');svg.style.height='auto';svg.style.display='block';drawMeasureLines(svg,timeline,ex);}
}
