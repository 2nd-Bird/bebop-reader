...
function enhancePractice(){
  ...
  btn.onclick=async()=>{
    ...
    if(state)state.innerHTML='<span class="pulse-dot gold"></span><b>お手本</b><small>1小節を目と耳でひとまとまりにする</small>';
    try{
      ph.style.left=`${playheadPercent(score,ex,0)}%`;ph.classList.add('active');
      await demoPhrase(ex,p=>{ph.style.left=`${playheadPercent(score,ex,p)}%`;});
    }catch(e){toast(e.message||String(e));}
    finally{
      ph.classList.remove('active');btn.disabled=false;if(sing)sing.disabled=false;btn.innerHTML=old;
      if(state)state.innerHTML='<span class="pulse-dot"></span><b>譜面から先に音を聴く</b><small>4カウント後、1小節を止まらず歌う</small>';
    }
  };
}
