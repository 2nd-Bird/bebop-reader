import { getAudioContext, getMasterBus, ensureAudio, primeAudio, audioStatus } from './audio/context.js';
...
function ping(freq,start,duration,volume=.25,type='sine'){const c=getAudioContext(),osc=c.createOscillator(),g=c.createGain();osc.type=type;osc.frequency.setValueAtTime(freq,start);g.gain.setValueAtTime(.0001,start);g.gain.exponentialRampToValueAtTime(Math.max(.0002,volume),start+.008);g.gain.setValueAtTime(Math.max(.0002,volume),Math.max(start+.01,start+duration-.045));g.gain.exponentialRampToValueAtTime(.0001,start+duration);osc.connect(g);g.connect(getMasterBus());osc.start(start);osc.stop(start+duration+.02);}
function wavUrl(ex){
...
export async function countIn(bpm,onBeat){await ensureAudio();const c=getAudioContext(),spb=60/bpm,base=c.currentTime+.04;for(let i=0;i<4;i++){const t=base+i*spb;ping(i===0?1174.66:880,t,.07,i===0?.36:.25,'square');setTimeout(()=>onBeat?.(i+1),Math.max(0,(t-c.currentTime)*1000));}await sleep(spb*4*1000+50);}
export { primeAudio, ensureAudio, audioStatus };
