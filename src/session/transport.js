...
export function createTransport({ audioContext, bpm, beatsPerBar = 4 }) {
  ...
  const requireStarted = () => { if (originTime == null) throw new Error('transport has not started'); };
  const resumeAt = (beat, leadSec) => {
    const target=Math.max(pausedBeat,Number(beat)||0);
    originTime=audioContext.currentTime+leadSec-target*secondsPerBeat;
    pausedBeat=target;state='running';return target;
  };
  ...
  return {
    ...
    startAt(audioTime) {
      ...
      originTime = audioTime; pausedBeat = 0; state = 'running'; return originTime;
    },
    ...
    timeAtBeat(beat) { requireStarted(); return originTime + beat * secondsPerBeat; },
    beatAtTime(audioTime) { requireStarted(); return (audioTime - originTime) / secondsPerBeat; },
    ...
    pause() {
      ...
      pausedBeat = this.currentBeat(audioContext.currentTime); state = 'paused'; return pausedBeat;
    },
    resumeAtBeat(beat,{leadSec=.06}={}) {
      if(state!=='paused')return this.currentBeat();
      if(!Number.isFinite(beat))throw new Error('resume beat must be finite');
      return resumeAt(beat,leadSec);
    },
