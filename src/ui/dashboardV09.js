...
import { schedulerSignals, cBluesStageReady } from '../curriculum/mastery.js';
import { nextCBluesFlowAction } from '../curriculum/flowProgress.js';
...
function keyWorldsHTML(state){
  const progress=state.keyProgress||{};
  return ['C','F','Bb'].map(key=>{const unlocked=key==='C'||Boolean(progress?.[key]?.unlocked),status=key==='C'?'BASE':unlocked?'OPEN':'LOCKED';return`<div class="world ${unlocked?'active':''}"><b>${keyLabel(key)}</b><span>Major</span><small>${status}</small></div>`;}).join('');
}
function currentSnapshot(state){
  ...
  return{stageNo,stage,families,due,weak,focus,readiness,forms,activeForm,stage14Ready,nextFlow};
}
export function renderV09Home({app,navigate}){
  ...
  const root=shell(app,`<section class="hero v09-hero"><div class="eyebrow">TODAY · 7 MIN</div><h1>音楽を止めずに、<br><em>読む。</em></h1><p>聴く → 見る → 頭で鳴らす → 歌う。失敗しても流れは止めず、あとで同じ動きへ戻る。</p></section><section class="today-card card v09-today"><div class="today-top"><div><span class="label">STAGE ${x.stageNo} · ${esc(x.stage.title)}</span><h2>${esc(focusText)}</h2></div><div class="tempo-badge"><span>♩</span><b>60</b></div></div><div class="v09-flow-strip"><span>LISTEN</span><i>→</i><span>SEE</span><i>→</i><span>SING</span><i>→</i><span>FLOW</span></div><div class="v09-gate"><small>次にできること</small><b>${esc(x.stage.gate)}</b></div><button class="primary big" id="v09-start">セッションを始める <span>→</span></button><small class="v09-duration">約5〜7分 · 音楽は止まりません</small></section><section class="metrics-grid"><div class="metric card"><span>STREAK</span><b>${state.streak||0}<small>日</small></b><em>${state.totalSessions||0} sessions</em></div><div class="metric card"><span>READINESS</span><b>${x.readiness}<small>%</small></b><em>${x.due.length?`${x.due.length} review due`:'on track'}</em></div></section>${last?`<section class="card v09-last"><span class="label">LAST SESSION</span><div><b>${last.stars||0}★</b><span>Reading ${last.readScore??'—'}</span><span>Pitch ${last.pitch??'—'}</span><span>Flow ${last.flow??'—'}</span></div></section>`:''}<section class="locked-worlds">${keyWorldsHTML(state)}</section>`,{active:'home'});
...
export function renderV09Progress({app,navigate}){
  ...
  const root=shell(app,`<section class="page-title"><span class="eyebrow">READING PROGRESS</span><h1>ジャズの線へ進む。</h1><p>同じ動きを、ヒントが消えても五線譜から歌えるようになると次へ進みます。</p></section><section class="mastery-hero card"><div class="big-number">${x.readiness}<small>%</small></div><div><span>STAGE ${x.stageNo}</span><h2>${esc(x.stage.title)}</h2><p>${esc(x.stage.gate)}</p></div></section><section class="locked-worlds">${keyWorldsHTML(state)}</section><div class="v09-stage-map">${stageNodes}</div><section class="v09-family-list"><div class="group-head"><div><h2>動き</h2><small>${x.due.length} review · ${x.weak.length} weak</small></div></div>${familyRows}</section>`,{active:'progress'});
  bindNav(root,navigate);
}
