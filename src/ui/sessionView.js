...
export function createSessionView({ app, navigate, key='C' }) {
  ...
  app.innerHTML = `<div class="app-shell is-compact session-v09"><header class="topbar"><button class="brand session-close"><span class="brand-mark">B</span><span><b>Bebop Reader</b><small>READ → SING → FLOW</small></span></button><div class="key-pill"><span>KEY</span><b>${initialKeyLabel}</b></div></header><main class="session-main"><div class="session-meta"><button class="icon-btn session-close">×</button><div class="session-progress"><div id="session-progress-fill"></div></div><span id="session-position">READY</span></div><section class="session-intro"><span class="eyebrow">CONTINUOUS SESSION</span><h1>音楽を止めずに読む。</h1><p>最初の4カウント後は、beatが流れ続けます。</p></section><section class="score-card score-card-v08 session-score-card"><div class="morph-badge hidden" id="session-morph"></div><div class="score-wrap" id="session-score-viewport"><div id="session-score"></div><div id="session-playhead" class="playhead"></div></div><div class="session-empty" id="session-empty">次の譜面は音楽の中で現れます</div></section><div class="session-phase" id="session-phase"><span class="pulse-dot"></span><b>READY</b><small>STARTを一度タップ</small></div><div class="session-count hidden" id="session-count">1</div><div class="session-feedback" id="session-feedback"></div><button class="primary big" id="session-start">START <span>→</span></button><button class="primary big hidden" id="session-resume">RESUME <span>→</span></button><div class="session-debug hidden" id="session-debug"></div></main><div class="build-tag">v0.9 slice</div></div>`;
  const root = app.querySelector('.session-v09'), score = root.querySelector('#session-score'), viewport = root.querySelector('#session-score-viewport'), playhead = root.querySelector('#session-playhead'), phase = root.querySelector('#session-phase'), count = root.querySelector('#session-count'), feedback = root.querySelector('#session-feedback'), start = root.querySelector('#session-start'), resume = root.querySelector('#session-resume'), progress = root.querySelector('#session-progress-fill'), position = root.querySelector('#session-position'), empty = root.querySelector('#session-empty'), debug = root.querySelector('#session-debug'), morph = root.querySelector('#session-morph'),keyPill=root.querySelector('.key-pill b');
  ...
  return {
    ...
    showSummary(summary) {
      document.body.classList.remove('attempting');playhead.classList.remove('active');root.querySelector('.session-main').innerHTML=sessionSummaryMarkup(summary);root.querySelector('#session-done').onclick=()=>navigate('/');
