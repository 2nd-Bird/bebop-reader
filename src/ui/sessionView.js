...
export function createSessionView({ app, navigate }) {
  ...
  return {
    ...
    update({ beat, bar, beatInBar, totalBeats, progress: noteProgress, event, phase: phaseName }) { const pct = Math.max(0, Math.min(100, beat / totalBeats * 100)); progress.style.width = `${pct}%`; position.textContent = beat < 0 ? 'COUNT IN' : `BAR ${bar} · ${beatInBar}`; if (currentEvent && currentScoreModel && event?.eventId === currentEvent.eventId) updateFollower(score, viewport, playhead, currentScoreModel, noteProgress, { autoScroll: true }); if (debugEnabled) debug.textContent = `beat ${beat.toFixed(2)} | bar ${bar}:${beatInBar} | ${event?.eventId || '-'} | ${event?.presentationMode||'-'} | ${phaseName}`; },
