...
function buildSlots(families,eventCount,{familyMastery={},dueFamilyIds=[],formIntegration=false}={}){
 ...
 for(let i=0;i<eventCount;i++){
  ...
  out.push({family,variant,mode,sequenceIndex,harmonyFieldId:entry.harmonyFieldId||null,tonalFieldId:entry.tonalFieldId||null,harmonyTransfer,tonalFieldTransfer});
 }
 return out;
}
...
export function buildDailySessionPlan({currentStage=0,key='C',bpm=60,eventCount=20,targetSessionBeats=320,dueFamilyIds=[],weakFamilyIds=[],familyMastery={},harmonyFieldOverrides={},formId=null}={}){
 ...
 const families=chooseFamilies({currentStage,dueFamilyIds,weakFamilyIds,preferredFamilyIds:musicalForm?.integrationFamilyIds||[]});
 ...
 const slots=buildSlots(families,eventCount,{familyMastery,dueFamilyIds,formIntegration:Boolean(musicalForm)});
 ...
 for(let i=0;i<slots.length;i++){
  const {family,variant,mode,sequenceIndex,harmonyFieldId:sequenceHarmonyFieldId,tonalFieldId,harmonyTransfer,tonalFieldTransfer}=slots[i],scoreBeats=variantBeats(variant),fieldBeats=fieldBeatsFor(scoreBeats);
  ...
  let singStartBeat;
  if(musicalForm){
    const offsets=musicalForm.eventSingOffsets?.length?musicalForm.eventSingOffsets:[8];
    singStartBeat=startBeat+offsets[i%offsets.length];
