import {foldMidiToTarget,targetAtTime} from './music/pitch.js';
...
export function renderPitchGuide(container,ex,result){
  ...
  const targetMidis=voiced.map(n=>n.midi);
  // Keep the visual scale anchored to the exercise. A bad octave estimate must not stretch the chart.
  const lo=Math.floor(Math.min(...targetMidis)-3),hi=Math.ceil(Math.max(...targetMidis)+3);
  const y=m=>top+(hi-clamp(m,lo,hi))/Math.max(1,hi-lo)*(H-top-bottom),x=t=>left+t/totalSec*(W-left-right);
  ...
  for(let i=1;i<trace.length;i++){
    const a=trace[i-1],b=trace[i];if(b.t-a.t>.12)continue;
    const target=targetAtTime(ex,(a.t+b.t)/2);if(!target)continue;
    const am=foldMidiToTarget(a.midi,target.midi),bm=foldMidiToTarget(b.midi,target.midi),err=Math.abs(((am+bm)/2-target.midi)*100),line=document.createElementNS(NS,'line');
    line.setAttribute('x1',x(a.t));line.setAttribute('y1',y(am));line.setAttribute('x2',x(b.t));line.setAttribute('y2',y(bm));line.setAttribute('class',err<=40?'sung-trace hit':err<=80?'sung-trace near':'sung-trace miss');svg.appendChild(line);
  }
