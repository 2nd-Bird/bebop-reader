...
const fourBarPattern=program.variantIds.join(',');
...
for(const [i,action] of ['CONNECT','TRADE','RECALL'].entries())flowRecord=applyEventResult(flowRecord,{...flowBase,flowAction:action},{readScore:88-i,stars:4},1000+i);
let flowEvidence={'g-to-f-surfaces':flowRecord};
assert(cBluesRecallReady(flowEvidence)&&!cBluesOneChorusReady(flowEvidence),'Connect → Trade → Recall unlocks one chorus but does not silently complete it');
...
flowRecord=applyEventResult(flowRecord,closing,{readScore:84,stars:4},2000);
flowEvidence={'g-to-f-surfaces':flowRecord};
assert(cBluesOneChorusReady(flowEvidence),'successful one-chorus read is recorded as its own FLOW evidence');
assert(flowRecord.flowActions.join(',')==='CONNECT,TRADE,RECALL,ONE_CHORUS','FLOW history preserves the progression into one chorus');
assert(flowRecord.coldReadAttempts===0&&flowRecord.coldVariantIds.length===0&&flowRecord.seenVariantIds.length===0,'composite one-chorus FLOW does not fabricate Variant or cold-read mastery');
assert(!cBluesStageReady(flowEvidence),'one-chorus FLOW alone still cannot bypass the required I/IV/V cold form transfer evidence');
...
assert(cBluesStageReady(evidence),'cold I/IV/V transfer plus the complete late FLOW sequence including one chorus completes the C Blues gate');
assert(recommendedFormIdForStage14(evidence)==='rhythm-changes-32','Rhythm Changes unlocks only after the learner has carried known material through one full Blues chorus');
...
assert(debugPlan.events.at(-1).flowAction==='ONE_CHORUS'&&debugPlan.events.at(-1).startBeat===32&&debugPlan.events.at(-1).singStartBeat===48&&debugPlan.events.at(-1).singEndBeat===96,'debug harness gives four bars of pre-read, then one complete chorus from the next downbeat');
...
assert(view.includes("event.flowAction==='ONE_CHORUS'?'1コーラスを読む'"),'learner-facing one-chorus copy describes the musical action without theory jargon');
assert(player.includes("['CONNECT','TRADE','RECALL','ONE_CHORUS']"),'debug harness can expose one chorus deliberately without changing normal mastery');

console.log('OK: Stage 14 grows the known four-bar CONNECT material into a fully visible 12-bar C Blues reading task with four bars of pre-read and 48 continuous SING beats');
