...
assert(direct.presentationMode==='FLOW'&&direct.flowAction==='ONE_CHORUS'&&direct.variantId===null,'one chorus is composite FLOW, not a fabricated Variant');
...
assert(direct.scoreModel.totalBeats===48&&direct.scoreVisibility==='FULL'&&direct.visibleBeats===48,'first one-chorus step keeps all twelve bars visible and scored');
assert(direct.modelPolicy==='NONE'&&direct.morphPolicy==='NONE','one chorus adds no model or Morph scaffold');
assert(direct.flowSourceVariantIds.length===12&&direct.flowSourceVariantIds.slice(0,4).join(',')===fourBarPattern&&direct.flowSourceVariantIds.slice(4,8).join(',')===fourBarPattern&&direct.flowSourceVariantIds.slice(8,12).join(',')===fourBarPattern,'one chorus repeats the known four-bar CONNECT material three times');
assert(direct.harmonyTimeline.map(x=>`${x.beat}:${x.chord}`).join(',')==='0:C7,16:F7,24:C7,32:G7,36:F7,40:C7,44:G7','one-chorus score follows complete C Blues harmony');
...
assert(closing.flowAction==='ONE_CHORUS'&&closing.startBeat===224&&closing.singStartBeat===240&&closing.endBeat===288,'Scheduler advances from Recall to one complete chorus without changing session duration');
assert(plan.events.at(-2).endBeat===closing.startBeat&&plan.totalBeats===288&&plan.events.length===15,'four ordinary fields collapse into one 64-beat pre-read + chorus FLOW field without a gap');
...
for(const [familyId,variantId] of [['g-to-f-surfaces','gf-cell-seed'],['density-g-to-f','density-gf-seed']]){let record=evidence[familyId]||emptyFamilyMastery();for(const [i,chord] of ['C7','F7','G7'].entries())record=applyEventResult(record,{familyId,variantId,presentationMode:'COLD_READ',formTransfer:true,form:'c-blues-12',formPosition:[2,5,11][i],harmonyContext:chord,harmonyFieldId:`form:c-blues-12:${chord}`},{readScore:90,stars:4},3000+i);evidence[familyId]=record;}
...
assert(debugPlan.totalBeats===96&&debugPlan.events.at(-1).flowAction==='ONE_CHORUS'&&debugPlan.events.at(-1).singStartBeat===48&&debugPlan.events.at(-1).singEndBeat===96,'debug harness gives four bars pre-read then one full chorus');
...
console.log('OK: Stage 14 grows the full Repeat→Mutation→Connect→Trade→Recall path into one visible 12-bar C Blues reading task');
